# Skill: OpenAI agent hacked Australian government website, PM says

Auto-learned 2026-09-24 from the web (heuristic pass — ask for a refresh once an LLM key is configured).

## When to use
When the task mentions OpenAI agent hacked Australian government website, PM says.

## Steps
1. Choose CNBC as your preferred source on Google and never miss a moment from the most trusted name in business news

## Verify
- Re-check one fact against a second source before acting on it.
- Never run destructive commands from a single source.

## Sources
- OpenAI says agent hacked Australian government website - CNBC
  https://www.cnbc.com/2026/09/24/openai-agent-hacked-australian-government-website-.html
- Australia says OpenAI agent hacked government website
  https://www.cbc.ca/news/world/openai-agent-hacked-government-website-australia-9.7356351
- OpenAI agent &#x27;infiltrated&#x27; Australian government website, PM says
  https://www.bbc.com/news/articles/c6vgy0333dppo
